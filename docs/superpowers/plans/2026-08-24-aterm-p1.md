...
rsync -a aterm-scaffold/ ./ --exclude node_modules --exclude .gitignore --exclude .git
rm -rf aterm-scaffold
# 注意：不覆盖仓库根已有的 .gitignore（它包含 .superpowers/ 等条目），缺的条目在 Step 4 手动合并
npm install
```
...
- [ ] **Step 5: 验证（全部无头执行，不开 GUI）**

Run: `cd src-tauri && cargo test && cd .. && npx vitest run && npm run build && npm run tauri build -- --debug`
Expected: cargo test 编译通过 0 failed；vitest "no tests" 通过；vite build 零错误；tauri debug 打包产出 `src-tauri/target/debug/bundle/macos/aTerm.app`。GUI 窗口的人工验证统一放在 Task 10 与用户一起做。
...
- [ ] **Step 3: 编译门槛 + 真机清单（清单本身移交 Task 10）**

本任务的完成门槛：`cd src-tauri && cargo check && cd .. && npx vitest run && npm run build` 全部零错误。
以下人工 GUI 清单**不在本任务执行**，由控制端在 Task 10 与用户一起批量验证（保留在此供 Task 10 引用）——
...
- [ ] **Step 5: 编译门槛 + 真机清单（清单本身移交 Task 10）**

本任务的完成门槛：`npx vitest run && npm run build` 零错误。
以下人工 GUI 清单**不在本任务执行**，由控制端在 Task 10 与用户一起批量验证——
