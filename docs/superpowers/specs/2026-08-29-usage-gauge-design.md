...
> **状态：已搁置，未实现（2026-08-29）。**
> 决定不做的理由：用户已有独立工具 bookholder 承担同一职责，为把同一份数据搬进
> aTerm 底栏而让本应用首次获得「联网 + 读取他人凭据」两项能力，代价与收益不成
> 比例。本文保留，是因为其中的调研结论（未公开接口的地址与 beta 头、钥匙串条目
> 结构、该通道的脆弱性、ETA 估算必须保留的两条守卫）都是实测所得，将来若重启
> 该功能可直接复用，无需重查。**在有人明确决定重启之前，不要按此文实施。**
