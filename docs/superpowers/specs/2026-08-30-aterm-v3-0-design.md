...
**追加的对照实验（2026-08-30 深夜）**：用 Python `pty.fork()` 起一个**双向真 pty**
（与 aTerm 的 portable-pty 条件相同）再试，claude 完整跑完一轮（屏幕上出现
`Worked for 3s · done`），仍无转录。随即做对照组——**同一脚本、唯一区别是去掉
`--session-id`**——同样无转录。同一时刻 `~/.claude/projects` 下其它真实会话的 jsonl
正在增长，证明实时写入本身是存在的。

**结论：夹具无法观测转录创建，因此它对这个 flag 没有任何证言**（不是"证明 flag 无效"，
而是"这个实验根本测不了 flag"）。先前那点疑似反面证据到此被证伪。

**处置**：走主路。依据是 `-p` 模式已证明机制存在、`--help` 未声明任何模式限制、唯一的
反面证据已被对照组排除。Task 1 Step 6 由**阻塞关卡**降级为**用户验收项**——被推翻时
只需改 `newConversationSpec` 一处，`resolvePaneIdentity` 与 `reconcilePanes` 两个核心件
在两条路上逐字相同（退路见下方 2.1-fallback）。
