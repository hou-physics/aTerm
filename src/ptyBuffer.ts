...
// 会触发 WKWebView OOM 的几百 MB 低一个数量级。
//
// 但要说准确：这里的「外来 PTY 数」是**累计**数，不是并发数。pty-exit（pty.rs:32，同样是
// 全应用广播）只 exited.add + 调 exitSink，**不删 buffers**，所以一个外来 PTY 退出之后它
// 那最多 2 MiB 仍然留着，永不释放。一天里在另一个窗口开关 50 个会话，本窗口就常驻约
// 100 MB。相比修复前（按输出量无界增长）低了一到两个数量级，且单窗口零影响（没有外来
// id），但**仍随会话数单调增长**——早先版本的注释写的是"不随时间增长"，那句话不准确。
//
// 为什么不直接"pty-exit 就删"：本窗口自己的 PTY 完全可能在 <TerminalView> 挂载之前就退出
// （比如一条瞬间失败的命令），那时删掉缓冲，用户看到的是一个全空的终端、连报错都没有。
// 安全的收法（退出后延迟释放，或再加一条全局总量上限）是另一个设计，不在 V3.3 范围内。
