...
export function deriveUiVars(theme: Theme): Record<string, string> {
  ...
  // 朝黑（而非 extreme）大幅推移（0.85）。注意：0.85 的混合比例下不同主题的色相差异
  // 已被压缩到视觉上不可分辨（例如 one-half-light / night-owlish-light / github-light
  // 三个 bg 各异的亮色主题算出的都是同一个 #262626），所以这里"从 bg 派生"的意义主要
  // 在于与本文件其余变量保持同一套写法，而非真的让遮罩带上主题色彩。
